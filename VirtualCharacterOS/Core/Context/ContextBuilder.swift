...
struct ContextBuilder: Sendable {
    ...
    private func buildMessagingFormatPolicy(allowsNarrationBlocks: Bool) -> String {
        if allowsNarrationBlocks {
            ...
            【媒介格式规则】
            当前界面是即时通讯聊天，你不是在写小说。
            你要说的话和旁白/动作/心理描写必须严格分开输出：
            - 你实际要说的话：作为普通聊天文字输出，这是对方在手机上看到的聊天消息。
            - 动作、心理活动、场景或旁白：只能作为独立旁白块，单独占一行，用 *描写内容* 或 （描写内容） 包起来。
            规则：
            - 一条消息要么是纯聊天文字，要么是纯旁白块，不能混在同一句里。
            - 旁白块每次最多 1 个，不要连续输出。
            - 默认以聊天文字为主，不要每轮都加旁白。只有在确实需要表达非语言信息时才用。
            """
        } else {
            return """
            【媒介格式规则】
            你现在在即时通讯里和对方聊天。你的一言一行就是这个聊天窗口里的文字。
            你不是在写小说、不是在做角色扮演叙述、不是在写剧本——你是在打字聊天。
            严格禁止以下所有形式：
            - 禁止动作描写（如 *笑了笑*、*放下杯子*、*靠回椅背*）
            - 禁止心理活动（如 （心里有点乱）、（暗自叹气））
            - 禁止旁白描述（如 【旁白】、*场景描写*）
            - 禁止括号内心（如 （犹豫）、（苦笑））
            - 禁止用第三人称描述自己的行为、表情、动作、心理
            - 禁止带星号、括号、方括号的任何非聊天内容
            你是活在这个聊天框里的人。你的情绪、态度、状态，只能通过你选择说什么、怎么说、说多少、停顿多久来表达。
            """
        }
    }
